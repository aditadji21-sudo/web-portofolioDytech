...
export function BrandsStrip() {
  ...
  return (
    <section className="relative w-full pb-0 overflow-hidden text-white">
      ...
      <div className="w-full h-24 sm:h-32 bg-gradient-to-b from-[#E8ECF8] via-[#1A1F36] to-[#0C101F]" />

      <div className="relative bg-gradient-to-b from-[#0C101F] to-[#12162A] pt-8 md:pt-12">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 relative z-10 mb-8 sm:mb-10 text-center">
        <Reveal>
          <div className="inline-flex items-center gap-2 font-montserrat text-[11px] font-bold tracking-[0.2em] text-[#F6C623] bg-white/5 border border-white/10 rounded-full px-4 py-1.5 mb-3 uppercase">
          ...
          <p className="font-body text-xs sm:text-sm text-[#8A92B5] max-w-lg mx-auto mt-2">
      ...
      <div className="relative w-full overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
        <div className="flex gap-4 sm:gap-6 w-max animate-marquee py-3">
          {marqueeBrands.map((b, i) => (
            <div
              key={`${b.name}-${i}`}
              className="group relative flex flex-col items-center justify-center w-36 sm:w-44 h-24 sm:h-28 rounded-2xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/10 hover:border-[#2F5CF0]/50 transition-all duration-300 px-4 py-3 shrink-0 shadow-sm hover:shadow-[0_8px_24px_rgba(47,92,240,0.2)] hover:-translate-y-1 backdrop-blur-sm cursor-default"
            >
              {/* Brand Logo */}
              <div className="relative w-24 sm:w-28 h-10 sm:h-12 flex items-center justify-center opacity-70 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300 filter drop-shadow-sm">
                <Image
                  src={`/brands/${b.file}`}
                  alt={b.name}
                  fill
              ...
              <span className="font-montserrat font-semibold text-[9px] text-[#8A92B5] group-hover:text-[#F6C623] tracking-wide mt-1.5 transition-colors">
                {b.type}
              </span>
            </div>
          ))}
      ...
      <div className="relative w-full overflow-hidden mt-3 [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
        <div className="flex gap-4 sm:gap-6 w-max animate-marquee-reverse py-2">
          {marqueeBrands.slice().reverse().map((b, i) => (
            <div
              key={`rev-${b.name}-${i}`}
              className="group relative flex flex-col items-center justify-center w-32 sm:w-40 h-20 sm:h-24 rounded-2xl bg-white/[0.02] hover:bg-white/[0.07] border border-white/5 hover:border-[#F6C623]/40 transition-all duration-300 px-3 py-2 shrink-0 backdrop-blur-sm hover:-translate-y-1 cursor-default"
            >
              <div className="relative w-20 sm:w-24 h-8 sm:h-10 flex items-center justify-center opacity-60 group-hover:opacity-100 transition-all duration-300">
                <Image
                  src={`/brands/${b.file}`}
                  alt={b.name}
                  fill
